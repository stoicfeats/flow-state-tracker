...
export const Stats: React.FC<StatsProps> = ({ sessions, layoutMode = 'standard' }) => {
  const stats = useMemo(() => {
    if (sessions.length === 0) return { currentStreak: 0, longestStreak: 0, totalHours: 0 };
    ...
    return { currentStreak: currentStreakCount, longestStreak: maxStreak, totalHours };
  }, [sessions]);

  return (
    <div className={`grid ${layoutMode === 'dashboard' ? 'grid-cols-1 gap-4' : 'grid-cols-3 gap-4 mb-8'}`}>
      <motion.div 
